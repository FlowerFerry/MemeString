...
inline namespace MMPP_NAMESPACE {
	...
	{
		MemeStringStack_init(&data_, MMSTR__OBJ_SIZE);
	}
	...
	MEMEPP__IMPL_INLINE string::string(native_handle_type&& _other)
	{
		...
		{
			*errc() = MemeStringStack_initByU8bytes(&data_, MMSTR__OBJ_SIZE,
				MemeString_byteData(other), MemeString_byteSize(other));
			MemeStringStack_unInit(&_other, MMSTR__OBJ_SIZE);
#if !MMOPT__EXCEPTION_DISABLED
            throw_errc(*errc());
#endif
		}
	...
	MEMEPP__IMPL_INLINE string::string(const native_handle_type& _other)
	{
		...
		if (MemeString_storageType(other) == MemeString_UnsafeStorageType_view)
		{
			...
			*errc() = MemeStringStack_initByOther(&data_, MMSTR__OBJ_SIZE, other);
		}
#if !MMOPT__EXCEPTION_DISABLED
		throw_errc(*errc());
#endif
	...
	MEMEPP__IMPL_INLINE string::string(const string & _other)
	{
		*errc() = MemeStringStack_initByOther(
			&data_, MMSTR__OBJ_SIZE, to_pointer(_other.data_));
#if !MMOPT__EXCEPTION_DISABLED
		throw_errc(*errc());
#endif
	}
	...
	{
		*errc() = MemeStringStack_initByU8bytesAndType(&data_, MMSTR__OBJ_SIZE,
	...
	{
		*errc() = MemeStringStack_initByU8bytes(&data_, MMSTR__OBJ_SIZE, _utf8, -1);
#if !MMOPT__EXCEPTION_DISABLED
		throw_errc(*errc());
#endif
	}
	...
	{
		*errc() = MemeStringStack_initByU8bytes(&data_, MMSTR__OBJ_SIZE, _utf8, _size);
#if !MMOPT__EXCEPTION_DISABLED
		throw_errc(*errc());
#endif
	}
	...
	{
        *errc() = MemeStringStack_initByU16bytes(&data_, MMSTR__OBJ_SIZE, _utf16, _size);
#if !MMOPT__EXCEPTION_DISABLED
        throw_errc(*errc());
#endif
	}
	...
	{
        *errc() = MemeStringStack_initByU16bytesAndType(&data_, MMSTR__OBJ_SIZE,
	...
	MEMEPP__IMPL_INLINE string::~string()
	{
		MemeStringStack_unInit(&data_, MMSTR__OBJ_SIZE);
	}
	...
	{
		*errc() = MemeStringStack_assign(&data_, MMSTR__OBJ_SIZE, to_pointer(_other.data_));
#if !MMOPT__EXCEPTION_DISABLED
		throw_errc(*errc());
#endif
		return *this;
	...
	MEMEPP__IMPL_INLINE rune string::rune_front() const noexcept
	{
        return MemeString_runeFront(to_pointer(data_));
	}
	
	MEMEPP__IMPL_INLINE rune string::rune_back() const noexcept
	{
	...
	MEMEPP__IMPL_INLINE string string::to_shared_storage() const noexcept
	{
		auto st = storage_type();
		if (st == string_storage_t::large || st == string_storage_t::user)
			return *this;
	...
	{
		return storage_type() == string_storage_t::small ? *this : to_shared_storage();
	}
	...
	{
		mmstrstk_t out;
		mgec_t ec = *errc() = MemeStringStack_mid_v2(
			&native_handle(), _pos, _count, &out, sizeof(out));
#if !MMOPT__EXCEPTION_DISABLED
		throw_errc(ec);
#endif
		return ec ? string{} : string{ std::move(out) };
	...
	{
		mmstrstk_t out;
		mgec_t ec = *errc() = MemeStringStack_concat_v2(
			&native_handle(), &(_other.native_handle()), &out, sizeof(out));
		if (ec)
			mmstrstk_uninit(&out);
#if !MMOPT__EXCEPTION_DISABLED
		throw_errc(ec);
	...
	{
		mmstrstk_t stk;
		mgec_t ec = *errc() = MemeStringStack_getRepeat_v2(
		...
		throw_errc(ec);
#endif
		return ec ? string{} : string{ std::move(stk) };
	}
	...
	MEMEPP__IMPL_INLINE bool operator==(const char* _lhs, const string& _rhs) noexcept
	{
		int result = 0;
		MemeString_isEqual(
	...
	MEMEPP__IMPL_INLINE bool operator==(const string& _lhs, const char* _rhs) noexcept
	{
		int result = 0;
		MemeString_isEqual(
	...
	MEMEPP__IMPL_INLINE bool operator==(const string& _lhs, string::const_pointer _rhs) noexcept
	{
        int result = 0;
        MemeString_isEqual(
	...
	MEMEPP__IMPL_INLINE bool operator==(string::const_pointer _lhs, const string& _rhs) noexcept
	{
        int result = 0;
        MemeString_isEqual(
	...
	MEMEPP__IMPL_INLINE bool operator==(const string& _lhs, const rune& _rhs) noexcept
	{
		int result = 0;
	...
	MEMEPP__IMPL_INLINE bool operator==(const rune& _lhs, const string& _rhs) noexcept
	{
		int result = 0;
	...
	MEMEPP__IMPL_INLINE bool operator<(const string& _lhs, const string& _rhs) noexcept
	{
		return MemeString_compare(
            to_pointer(_lhs.native_handle()), to_pointer(_rhs.native_handle())) < 0;
	}
	...
	MEMEPP__IMPL_INLINE bool operator<(const string& _lhs, const char* _rhs) noexcept
	{
        return MemeString_compareByUtf8bytes(
            to_pointer(_lhs.native_handle()), 
			reinterpret_cast<string::const_pointer>(_rhs), -1) < 0;
	}
	...
	MEMEPP__IMPL_INLINE bool operator<(const char* _lhs, const string& _rhs) noexcept
	{
        return MemeString_compareByUtf8bytes(
			...
			reinterpret_cast<string::const_pointer>(_lhs), -1) > 0;
	}
	...
	{
        return MemeString_compareByUtf8bytes(
            to_pointer(_lhs.native_handle()), _rhs, -1) < 0;
	}

	MEMEPP__IMPL_INLINE bool operator<(string::const_pointer _lhs, const string& _rhs) noexcept
	{
        return MemeString_compareByUtf8bytes(
            to_pointer(_rhs.native_handle()), _lhs, -1) > 0;
	}
	...
	MEMEPP__IMPL_INLINE bool operator<(const string& _lhs, const rune& _rhs) noexcept
	{
        return MemeString_compareByUtf8bytes(to_pointer(_lhs.native_handle()), 
            reinterpret_cast<string::const_pointer>(_rhs.data()), _rhs.size()) < 0;
	}
	
	MEMEPP__IMPL_INLINE bool operator<(const rune& _lhs, const string& _rhs) noexcept
	{
        return MemeString_compareByUtf8bytes(to_pointer(_rhs.native_handle()),
            reinterpret_cast<string::const_pointer>(_lhs.data()), _lhs.size()) > 0;
	}
	...
	MEMEPP__IMPL_INLINE string c_format(size_t _size_limit, MEGO_SYMBOL__MSVC_FORMAT_STRING(const char* _fmt),  ...)
	{
		...
		mgec_t ec = *errc() = 
			MemeStringStack_vformatWithLimitInCstyle_v2(
				&out, sizeof(out), _size_limit, -1, _fmt, args);
        va_end(args);
#if !MMOPT__EXCEPTION_DISABLED
		throw_errc(ec);
#endif
		...
		...)
	{
		...
		mgec_t ec = *errc() =
			MemeStringStack_vformatWithLimitInCstyle_v2(
				&out, sizeof(out), _size_limit, _predict, _fmt, args);
		va_end(args);
#if !MMOPT__EXCEPTION_DISABLED
		throw_errc(ec);
#endif

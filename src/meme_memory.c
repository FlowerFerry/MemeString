
#include <meme/string_memory.h>
#include <meme/impl/string_memory.h>

#include <mego/predef/endian.h>
#include <mego/util/endian/byte_swap.h>
#include <mego/err/ec.h>

#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
#include <errno.h>
#include <time.h>

void* __MemeMemory_Malloc(size_t _size)
{
    return malloc(_size);
}

void __MemeMemory_Free(void* _ptr)
{
    free(_ptr);
}

void* __MemeMemory_Calloc(size_t _count, size_t _size)
{
    return calloc(_count, _size);
}

void* __MemeMemory_Realloc(void* _ptr, size_t _size)
{
    return realloc(_ptr, _size);
}

MemeMemory_MallocFunc_t** __MemeMemory_MallocFuncObject()
{
	static MemeMemory_MallocFunc_t* pointer =
#if MMOPT__HEADTAIL_MEMCHECK_ENABLED
		&MemeMemoryCheck_Malloc;
#else
		&__MemeMemory_Malloc;
#endif
	return &pointer;
}

MemeMemory_FreeFunc_t** __MemeMemory_FreeFuncObject()
{
	static MemeMemory_FreeFunc_t* pointer =
#if MMOPT__HEADTAIL_MEMCHECK_ENABLED
		&MemeMemoryCheck_Free;
#else
		&__MemeMemory_Free;
#endif
	return &pointer;
}

MEME_EXTERN_C MEME_API mmsmem_malloc_t* MEME_STDCALL mmsmem_get_malloc_func()
{
	return *__MemeMemory_MallocFuncObject();
}

MEME_EXTERN_C MEME_API mmsmem_calloc_t* MEME_STDCALL mmsmem_get_calloc_func()
{
#if MMOPT__HEADTAIL_MEMCHECK_ENABLED
	return MemeMemoryCheck_Calloc;
#else
	return __MemeMemory_Calloc;
#endif
}

MEME_EXTERN_C MEME_API mmsmem_realloc_t* MEME_STDCALL mmsmem_get_realloc_func()
{
#if MMOPT__HEADTAIL_MEMCHECK_ENABLED
	return MemeMemoryCheck_Realloc;
#else
	return __MemeMemory_Realloc;
#endif
}

MEME_EXTERN_C MEME_API mmmem_free_fn_t* MEME_STDCALL mmsmem_get_free_func()
{
	return *__MemeMemory_FreeFuncObject();
}


mmmem_malloc_fn_t* mmmem_get_malloc_func()
{
	return *__MemeMemory_MallocFuncObject();
}

mmmem_calloc_fn_t* mmmem_get_calloc_func()
{
#if MMOPT__HEADTAIL_MEMCHECK_ENABLED
	return MemeMemoryCheck_Calloc;
#else
	return __MemeMemory_Calloc;
#endif
}

mmmem_realloc_fn_t* mmmem_get_realloc_func()
{
#if MMOPT__HEADTAIL_MEMCHECK_ENABLED
	return MemeMemoryCheck_Realloc;
#else
	return __MemeMemory_Realloc;
#endif
}

mmmem_free_fn_t* mmmem_get_free_func()
{
	return *__MemeMemory_FreeFuncObject();
}


#if MMOPT__HEADTAIL_MEMCHECK_ENABLED

//const static uint16_t __mmcheck_crc16_table[256] = {
//	0x0000,0xC0C1,0xC181,0x0140,0xC301,0x03C0,0x0280,0xC241,
//	0xC601,0x06C0,0x0780,0xC741,0x0500,0xC5C1,0xC481,0x0440,
//	0xCC01,0x0CC0,0x0D80,0xCD41,0x0F00,0xCFC1,0xCE81,0x0E40,
//	0x0A00,0xCAC1,0xCB81,0x0B40,0xC901,0x09C0,0x0880,0xC841,
//	0xD801,0x18C0,0x1980,0xD941,0x1B00,0xDBC1,0xDA81,0x1A40,
//	0x1E00,0xDEC1,0xDF81,0x1F40,0xDD01,0x1DC0,0x1C80,0xDC41,
//	0x1400,0xD4C1,0xD581,0x1540,0xD701,0x17C0,0x1680,0xD641,
//	0xD201,0x12C0,0x1380,0xD341,0x1100,0xD1C1,0xD081,0x1040,
//	0xF001,0x30C0,0x3180,0xF141,0x3300,0xF3C1,0xF281,0x3240,
//	0x3600,0xF6C1,0xF781,0x3740,0xF501,0x35C0,0x3480,0xF441,
//	0x3C00,0xFCC1,0xFD81,0x3D40,0xFF01,0x3FC0,0x3E80,0xFE41,
//	0xFA01,0x3AC0,0x3B80,0xFB41,0x3900,0xF9C1,0xF881,0x3840,
//	0x2800,0xE8C1,0xE981,0x2940,0xEB01,0x2BC0,0x2A80,0xEA41,
//	0xEE01,0x2EC0,0x2F80,0xEF41,0x2D00,0xEDC1,0xEC81,0x2C40,
//	0xE401,0x24C0,0x2580,0xE541,0x2700,0xE7C1,0xE681,0x2640,
//	0x2200,0xE2C1,0xE381,0x2340,0xE101,0x21C0,0x2080,0xE041,
//	0xA001,0x60C0,0x6180,0xA141,0x6300,0xA3C1,0xA281,0x6240,
//	0x6600,0xA6C1,0xA781,0x6740,0xA501,0x65C0,0x6480,0xA441,
//	0x6C00,0xACC1,0xAD81,0x6D40,0xAF01,0x6FC0,0x6E80,0xAE41,
//	0xAA01,0x6AC0,0x6B80,0xAB41,0x6900,0xA9C1,0xA881,0x6840,
//	0x7800,0xB8C1,0xB981,0x7940,0xBB01,0x7BC0,0x7A80,0xBA41,
//	0xBE01,0x7EC0,0x7F80,0xBF41,0x7D00,0xBDC1,0xBC81,0x7C40,
//	0xB401,0x74C0,0x7580,0xB541,0x7700,0xB7C1,0xB681,0x7640,
//	0x7200,0xB2C1,0xB381,0x7340,0xB101,0x71C0,0x7080,0xB041,
//	0x5000,0x90C1,0x9181,0x5140,0x9301,0x53C0,0x5280,0x9241,
//	0x9601,0x56C0,0x5780,0x9741,0x5500,0x95C1,0x9481,0x5440,
//	0x9C01,0x5CC0,0x5D80,0x9D41,0x5F00,0x9FC1,0x9E81,0x5E40,
//	0x5A00,0x9AC1,0x9B81,0x5B40,0x9901,0x59C0,0x5880,0x9841,
//	0x8801,0x48C0,0x4980,0x8941,0x4B00,0x8BC1,0x8A81,0x4A40,
//	0x4E00,0x8EC1,0x8F81,0x4F40,0x8D01,0x4DC0,0x4C80,0x8C41,
//	0x4400,0x84C1,0x8581,0x4540,0x8701,0x47C0,0x4680,0x8641,
//	0x8201,0x42C0,0x4380,0x8341,0x4100,0x81C1,0x8081,0x4040
//};

inline const uint16_t* MemeCheck_Crc16Table()
{
	const static uint16_t table[256] = {
	0x0000,0xC0C1,0xC181,0x0140,0xC301,0x03C0,0x0280,0xC241,
	0xC601,0x06C0,0x0780,0xC741,0x0500,0xC5C1,0xC481,0x0440,
	0xCC01,0x0CC0,0x0D80,0xCD41,0x0F00,0xCFC1,0xCE81,0x0E40,
	0x0A00,0xCAC1,0xCB81,0x0B40,0xC901,0x09C0,0x0880,0xC841,
	0xD801,0x18C0,0x1980,0xD941,0x1B00,0xDBC1,0xDA81,0x1A40,
	0x1E00,0xDEC1,0xDF81,0x1F40,0xDD01,0x1DC0,0x1C80,0xDC41,
	0x1400,0xD4C1,0xD581,0x1540,0xD701,0x17C0,0x1680,0xD641,
	0xD201,0x12C0,0x1380,0xD341,0x1100,0xD1C1,0xD081,0x1040,
	0xF001,0x30C0,0x3180,0xF141,0x3300,0xF3C1,0xF281,0x3240,
	0x3600,0xF6C1,0xF781,0x3740,0xF501,0x35C0,0x3480,0xF441,
	0x3C00,0xFCC1,0xFD81,0x3D40,0xFF01,0x3FC0,0x3E80,0xFE41,
	0xFA01,0x3AC0,0x3B80,0xFB41,0x3900,0xF9C1,0xF881,0x3840,
	0x2800,0xE8C1,0xE981,0x2940,0xEB01,0x2BC0,0x2A80,0xEA41,
	0xEE01,0x2EC0,0x2F80,0xEF41,0x2D00,0xEDC1,0xEC81,0x2C40,
	0xE401,0x24C0,0x2580,0xE541,0x2700,0xE7C1,0xE681,0x2640,
	0x2200,0xE2C1,0xE381,0x2340,0xE101,0x21C0,0x2080,0xE041,
	0xA001,0x60C0,0x6180,0xA141,0x6300,0xA3C1,0xA281,0x6240,
	0x6600,0xA6C1,0xA781,0x6740,0xA501,0x65C0,0x6480,0xA441,
	0x6C00,0xACC1,0xAD81,0x6D40,0xAF01,0x6FC0,0x6E80,0xAE41,
	0xAA01,0x6AC0,0x6B80,0xAB41,0x6900,0xA9C1,0xA881,0x6840,
	0x7800,0xB8C1,0xB981,0x7940,0xBB01,0x7BC0,0x7A80,0xBA41,
	0xBE01,0x7EC0,0x7F80,0xBF41,0x7D00,0xBDC1,0xBC81,0x7C40,
	0xB401,0x74C0,0x7580,0xB541,0x7700,0xB7C1,0xB681,0x7640,
	0x7200,0xB2C1,0xB381,0x7340,0xB101,0x71C0,0x7080,0xB041,
	0x5000,0x90C1,0x9181,0x5140,0x9301,0x53C0,0x5280,0x9241,
	0x9601,0x56C0,0x5780,0x9741,0x5500,0x95C1,0x9481,0x5440,
	0x9C01,0x5CC0,0x5D80,0x9D41,0x5F00,0x9FC1,0x9E81,0x5E40,
	0x5A00,0x9AC1,0x9B81,0x5B40,0x9901,0x59C0,0x5880,0x9841,
	0x8801,0x48C0,0x4980,0x8941,0x4B00,0x8BC1,0x8A81,0x4A40,
	0x4E00,0x8EC1,0x8F81,0x4F40,0x8D01,0x4DC0,0x4C80,0x8C41,
	0x4400,0x84C1,0x8581,0x4540,0x8701,0x47C0,0x4680,0x8641,
	0x8201,0x42C0,0x4380,0x8341,0x4100,0x81C1,0x8081,0x4040
	};
	return table;
}

uint16_t MemeCheck_Crc16le(const uint8_t* _buf, size_t _len)
{
    const uint16_t* table = MemeCheck_Crc16Table();
	uint16_t crc = 0xFFFF;
	if (!_buf)
		return crc;

	while (_len-- > 0)
		crc = table[(crc & 0xFF) ^ (*_buf++)] ^ (crc >> 8);

#if MEGO_ENDIAN__LITTLE_BYTE
	return crc;
#elif MEGO_ENDIAN__BIG_BYTE
	return MegoEndian_byteSwap16(crc);
#else
#	error "unknown"
#endif
}

void MemeCheckHead_init(MemeCheckHead_t* _factor)
{
	srand((unsigned int)time(0));

	_factor->count   = 0;
	_factor->byte[0] = (uint8_t)rand();
    _factor->byte[1] = (uint8_t)rand();
    _factor->byte[2] = (uint8_t)rand();
    _factor->byte[3] = (uint8_t)rand();
    _factor->byte[4] = (uint8_t)rand();
    _factor->byte[5] = (uint8_t)rand();
    
    _factor->crc = MemeCheck_Crc16le(_factor->byte, sizeof(_factor->byte));
}

void MemeCheckTail_init(MemeCheckTail_t* _factor)
{
	srand((unsigned int)time(0));

    _factor->byte[0] = (uint8_t)rand();
    _factor->byte[1] = (uint8_t)rand();
    _factor->byte[2] = (uint8_t)rand();
    _factor->byte[3] = (uint8_t)rand();
    _factor->byte[4] = (uint8_t)rand();
    _factor->byte[5] = (uint8_t)rand();

    _factor->crc = MemeCheck_Crc16le(_factor->byte, sizeof(_factor->byte));
}

MemeInteger_t MemeCheckHead_calibrate(const MemeCheckHead_t* _factor)
{
    return _factor->crc == MemeCheck_Crc16le(_factor->byte, sizeof(_factor->byte)) ?
		1 : 0;
}

MemeInteger_t MemeCheckTail_calibrate(const MemeCheckTail_t* _factor)
{
    return _factor->crc == MemeCheck_Crc16le(_factor->byte, sizeof(_factor->byte)) ?
		1 : 0;
}

const uint8_t* MemeCheck_getHeadPointer(const void* _data)
{
	if (!_data)
		return NULL;
	else
		return (const uint8_t*)_data - sizeof(MemeCheckHead_t);
}

const uint8_t* MemeCheck_getTailPointer(const void* _data)
{
	if (!_data)
        return NULL;
	else {
		MemeCheckHead_t* head = (MemeCheckHead_t*)MemeCheck_getHeadPointer(_data);
		return (const uint8_t*)_data + head->count - sizeof(MemeCheckHead_t) - sizeof(MemeCheckTail_t);
	}
}

MemeInteger_t MemeCheck_calibrate(const void* _data)
{
	MemeCheckHead_t* head; 
	MemeCheckTail_t* tail;
    
	if (mmsmem_get_malloc_func() != &MemeMemoryCheck_Malloc)
		return 1;

	head = (MemeCheckHead_t*)MemeCheck_getHeadPointer(_data);
	tail = (MemeCheckTail_t*)MemeCheck_getTailPointer(_data);

    return MemeCheckHead_calibrate(head) && MemeCheckTail_calibrate(tail) ?
        1 : 0;
}

void* MemeMemoryCheck_Malloc(size_t _size)
{
	MemeCheckHead_t* head;
	MemeCheckTail_t* tail;
    
	uint8_t * p = (uint8_t*)malloc(_size + sizeof(MemeCheckTail_t) + sizeof(MemeCheckHead_t));
	if (!p)
		return NULL;

    head = (MemeCheckHead_t*)p;
	MemeCheckHead_init(head);
	head->count = _size + sizeof(MemeCheckTail_t) + sizeof(MemeCheckHead_t);

    tail = (MemeCheckTail_t*)(p + head->count - sizeof(MemeCheckTail_t));
    MemeCheckTail_init(tail);

    return p + sizeof(MemeCheckHead_t);
}

void* MemeMemoryCheck_Realloc(void* _block, size_t _size)
{
	MemeCheckHead_t* head;
	MemeCheckTail_t* tail;

    if (_size == 0)
        return realloc(_block, 0);

	uint8_t* p = (uint8_t*)realloc((void*)MemeCheck_getHeadPointer(_block),
        _size + sizeof(MemeCheckTail_t) + sizeof(MemeCheckHead_t));
    if (!p)
        return NULL;

    head = (MemeCheckHead_t*)p;
	MemeCheckHead_init(head);
    head->count = _size + sizeof(MemeCheckTail_t) + sizeof(MemeCheckHead_t);
    
    tail = (MemeCheckTail_t*)(p + head->count - sizeof(MemeCheckTail_t));
    MemeCheckTail_init(tail);

    return p + sizeof(MemeCheckHead_t);
}

void* MemeMemoryCheck_Calloc(size_t _count, size_t _size)
{
	MemeCheckHead_t* head;
	MemeCheckTail_t* tail;

	if (_size == 0 || _count == 0)
		return NULL;
    _count += ((sizeof(MemeCheckTail_t) + sizeof(MemeCheckHead_t)) / _size + 1);
    
	uint8_t* p = (uint8_t*)calloc(_count, _size);
	if (!p)
		return NULL;

	head = (MemeCheckHead_t*)p;
	MemeCheckHead_init(head);
	head->count = _count * _size;

	tail = (MemeCheckTail_t*)(p + head->count - sizeof(MemeCheckTail_t));
	MemeCheckTail_init(tail);

	return p + sizeof(MemeCheckHead_t);
}

void MemeMemoryCheck_Free(void* _pointer)
{
	free((void*)MemeCheck_getHeadPointer(_pointer));
}
#endif

int MemeStringMemory_allocFunctionSwitch(
	mmmem_malloc_fn_t*  _in_malloc_fn , mmmem_free_fn_t* _in_free_fn,
	mmmem_malloc_fn_t** _out_malloc_fn, mmmem_free_fn_t** _out_free_fn)
{
	if (_in_malloc_fn == NULL || _in_free_fn == NULL) 
	{
		*_out_malloc_fn = mmsmem_get_malloc_func();
        *_out_free_fn   = mmsmem_get_free_func();
		return 0;
	}
	else {
        *_out_malloc_fn = _in_malloc_fn;
        *_out_free_fn   = _in_free_fn;
        return 0;
	}
}

MEME_EXTERN_C MEME_API void* MEME_STDCALL mmsmem_malloc(size_t _size)
{
	mmmem_malloc_fn_t* fn = mmsmem_get_malloc_func();
    return fn(_size);
}

MEME_EXTERN_C MEME_API void* MEME_STDCALL mmsmem_realloc(void* _block, size_t _size)
{
    mmmem_realloc_fn_t* fn = mmsmem_get_realloc_func();
    return fn(_block, _size);
}

MEME_EXTERN_C MEME_API void* MEME_STDCALL mmsmem_calloc(size_t _count, size_t _size)
{
    mmmem_calloc_fn_t* fn = mmsmem_get_calloc_func();
    return fn(_count, _size);
}

MEME_EXTERN_C MEME_API void MEME_STDCALL mmsmem_free(void* _pointer)
{
    mmmem_free_fn_t* fn = mmsmem_get_free_func();
	fn(_pointer);
}


MEME_EXTERN_C MEME_API void*
MEME_STDCALL mmmem_malloc(size_t _size)
{
	mmmem_malloc_fn_t* fn = mmmem_get_malloc_func();
	return fn(_size);
}

MEME_EXTERN_C MEME_API void*
MEME_STDCALL mmmem_calloc(size_t _count, size_t _size)
{
	mmmem_calloc_fn_t* fn = mmmem_get_calloc_func();
	return fn(_count, _size);
}

MEME_EXTERN_C MEME_API void*
MEME_STDCALL mmmem_realloc(void* _ptr, size_t _size)
{
	mmmem_realloc_fn_t* fn = mmmem_get_realloc_func();
	return fn(_ptr, _size);
}

MEME_EXTERN_C MEME_API void
MEME_STDCALL mmmem_free(void* _ptr)
{
	mmmem_free_fn_t* fn = mmmem_get_free_func();
	fn(_ptr);
}
